"""
Implementation of UUID v7 per the October 2021 draft update to RFC4122 from 2005:
<https://datatracker.ietf.org/doc/html/draft-peabody-dispatch-new-uuid-format>.

Stephen Simmons, 2021-12-27
"""

__all__ = ('uuid7', 'check_timing_precision', 'timestamp_ns', 'uuid_to_datetime')
...
def uuid7(
    ...
    _last=[0,0,0,0], _last_as_of=[0,0,0,0],
    ...
    with a potential time resolution of 50ns (if the system clock support this).

    Parameters
    ----------
    
    ns      - Optional integer setting the "as of" timestamp. 
              Expressed as the whole number of nanoseconds since Unix epoch.
              As a special case, uuid7(ns=0) returns a UUID with all zeros.
    ...
    time_func - Set the time function, which must return the integer
              number of nanoseconds since the Unix epoch, midnight on 1-Jan-1970.
              Defaults to time.time_ns(). This is exposed because
              time.time_ns() may have an unexpectedly low resolution on Windows.

    _last and _last_as_of - Used internally to trigger incrementing
              a sequence counter when consecutive calls have the same time values.
              The four values are [t1, t2, t3, seq], as described below.
              
    Returns
    -------
    
    A UUID object, or if as_type is specified, a string, int or bytes of length 16. 
    ...
    if ns == 0:
        ...
        # Treat the first 8 bytes of the uuid as a long (t1) and two ints (t2 and t3)
        # holding 36 bits of whole seconds and 24 bits of fractional seconds.
        ...
        # The next two bytes are an int (t4) with two bits for the variant 2 and 
        # a 14 bit sequence counter which increments if the time is unchanged.
    ...
    uuid_int = (t1 << 96) + (t2 << 80) + (t3 << 64) + (t4 << 48) + int.from_bytes(rand, 'big')
    if as_type == 'int':
        return uuid_int
    elif as_type == 'hex':
...
def check_timing_precision(timing_func: Optional[Callable[[], int]] = None) -> str:
    """
    ...
    timing_funcs = [
        ...
        ('datetime.datetime.utcnow', lambda: int(datetime.datetime.utcnow().timestamp() * 1_000_000_000)),
    ]
    ...
    for desc, fn in timing_funcs:
        ...
        lines.append(f"{desc} has a timing precision of {precision_ns:0,.0f}ns rather than {ideal_precision_ns:0,.0f}ns ({ctr:,} samples of which {len(values):,} are distinct, in {elapsed_ns / 1_000_000_000:0.2f}s)")
    
    return '\n'.join(lines)


def timestamp_ns(s: Union[str, uuid.UUID, int],
        suppress_error=True) -> Optional[int]:
    """
    ...
    if uuid_version == 7:
        ...
        frac_ns, rest = divmod(frac_binary * 1_000_000_000, 1 << 38)
        ns_since_epoch = whole_secs * 1_000_000_000 + frac_ns
        return ns_since_epoch
    elif suppress_error:
        return None
    else:
        raise ValueError(f"{str(s)} is a version {uuid_version} UUID, not v7 so we cannot extract the timestamp.")
...
def uuid_to_datetime(
    ...
    if ns_since_epoch is None:
        ...
        return datetime.datetime.fromtimestamp(ns_since_epoch / 1_000_000_000, tz=datetime.timezone.utc)
...
def test_uuid7():
    ...
    for n in range(100_000):
        i = uuid7str()
        if n > 0 and i <= last:
            raise RuntimeError(f"UUIDs are not monotonic:\n{last}\n{i}")
